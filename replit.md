...
## Persistence
`/home/runner` is a **volatile overlayfs** — it is completely wiped on every container restart. The only truly durable storage is `/home/runner/workspace`, which is a real **btrfs block device** that survives restarts.

`start.sh` sets up symlinks on every boot so apps see their expected paths:

| App path (symlink, volatile) | Real location (btrfs, persistent) |
|---|---|
| `~/Downloads` | `workspace/persistent/Downloads/` |
| `~/.local/share/blobevm-chrome` (Chromium profile) | `workspace/persistent/chromium-profile/` |

- `workspace/persistent/` is listed in `.gitignore` — large files (ROMs, downloads) are not committed to git, but they DO survive container restarts.
- **Do not store persistent user data anywhere under `$HOME` directly** — the volatile overlayfs will lose it on the next container restart.
- **Runtime-only files** (logs, sockets, fluxbox launcher scripts, novnc symlinks): live in `/tmp` and are intentionally regenerated by `start.sh` on every boot.
